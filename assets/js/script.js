...
let fcTemperature = document.querySelector(
...
let getWeatherByCityName = async (city) => {
  ...
  let endpoint = weatherBaseEndpoint
  ...
  let currentTemperatureFahrenheit =
    ((currentTemperatureKelvin - 273.15) * 9) / 5 + 32;
  ...
  for (let i = 1; i < 6; i++) {
    ...
    let forecastTemperatureFahrenheit =
      ((forecastTemperatureKelvin - 273.15) * 9) / 5 + 32;
    ...
    forecastItem.querySelector(".weatherForecastWind .value").textContent =
      forecastWindSpeed;
    forecastItem.querySelector(".weatherForecastHumidity .value").textContent =
      forecastHumidity;
    forecastItem
      .querySelector(".weatherForecastIcon")
      .classList.add(`wi-owm-${forecastIconCode}`);
  }
};
